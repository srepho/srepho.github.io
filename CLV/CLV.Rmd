...
Predictions about can then be fed back into other models or used directly on marketing decisions to try and encourage clients in danger to stay. One could model the purchases using similar methods.  
...
Some of the previously discussed models can be made to work in at least some of these cases (in particular Markov Chains). A lot of work has gone into looking at the problem more directly. It has lead to a whole branch of specialist literature.
...
# Pareto and friends
Most of the approaches treat as two separate problems:

1. The prediction of "Alive" or "Dead"
2. The probability of purchase/s

Starting with Ehrenberg (1959) a lot of literature centers around fitting distributions to these two questions. Ehrenberg proposed a Negative Binomial Distribution (NBD) but most modern approaches use two separate families of distributions; the Pareto/NBD model of Schmittlein et al (1987), the Beta-Geometric(BG)/NBD of Fader, Hardie & Lee (2005), the Central Variant of the Beta-Geometric (CBG)/NBD and CBG/CNBD-k based on Platzer (2008).

One useful analogy (taken from Medri 2012) is to imagine each customer as having two separate coins one that they flip to "decide" to answer 1. and the other for 2. (if applicable). The job of the data analyst is to predict these probabilities.
...
One of the great things about even these more sophisticated methods is that they are possible in Excel. This allows access to them to those who are not comfortable with a programming language. [Bruce Hardie](http://www.brucehardie.com/notes/) has a number of excel spreadsheets and excel walkthroughs of many of these models available. I will therefore just (non-exhaustively) list them here:
